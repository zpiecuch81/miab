...
public class MessageViewActivity extends Activity
{
  ...
  protected void onCreate( Bundle bundle )
  {
    ...
    todoUri = ( bundle == null )
        ? null : (Uri)bundle.getParcelable( MIABContentProvider.CONTENT_ITEM_TYPE );
    // Or passed from the other activity
    if( extras != null ) {
      todoUri = extras.getParcelable( MIABContentProvider.CONTENT_ITEM_TYPE );
      processMIAB( todoUri );
    }
  }
  ...
  private void setBackground( Cursor cursor )
  {
    int flags =
        cursor.getInt( cursor
            .getColumnIndexOrThrow( MIABSQLiteHelper.COLUMN_MESSAGE_FLAG ) );
  ...
  {
    messageView.setText(
        cursor.getString( cursor.getColumnIndexOrThrow( MIABSQLiteHelper.COLUMN_MESSAGE ) ) );
    int flags =
        cursor.getInt( cursor.getColumnIndexOrThrow( MIABSQLiteHelper.COLUMN_MESSAGE_FLAG ) );
    StringBuilder messageBuilder = new StringBuilder();
    messageBuilder
        .append( getString( R.string.msgMessageLeft ) )
        .append( new java.util.Date(
            cursor.getLong( cursor.getColumnIndexOrThrow(
                MIABSQLiteHelper.COLUMN_DROP_TIME_STAMP ) ) ).toString() )
        .append( "\n" + getString( R.string.msgMessageFound ) )
        .append( new java.util.Date(
            cursor.getLong( cursor.getColumnIndexOrThrow(
                MIABSQLiteHelper.COLUMN_FOUND_TIME_STAMP ) ) ).toString() )
