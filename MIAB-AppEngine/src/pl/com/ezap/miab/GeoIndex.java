...
/** @author zap
 * 
 *         This class is used to generate geometric index based on latitude and
 *         longitude. The GeoIndex is index of location area on earth's surface.
 *         All locations that are in the same area will have the same index,
 *         thus it's easier to search near by locations.
 * 
 *         First earth is cut in slices from south to north. These virtual cuts
 *         are done every DELTA_LATITUDE angle value, this way cut perimeter's
 *         length is about 0.5km. Then the slices are further cut based on the
 *         longitude value. */
public class GeoIndex
{
  /** This array holds ranges in which longitude has specific delta value. ( 75,
   * infinity ) - delta is 4.5 degree [ 60, 75 ) - delta is 0.045 degree [-60,
   * 60 ) - delta is 0.0045 degree [-75, -60 ) - delta is 0.045 degree [-91, -75
   * ) - delta is 4.5 degree
   * 
   * -91 is actually invalid (min. can be -90 on south pole) but that's just to
   * be sure */
  ...
  /** @param latitude
   *          in degrees, valid range [-90, 90]
   * @param longitude
   *          in degrees, valid range [-180,180]
   * @return geographical location index that is used to geographically makes
   *         locations closer */
